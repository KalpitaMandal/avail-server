...
const kalypsoConfig = {
  ...
  generatorEnclave: {
    url: "http://13.201.135.26:5000",
    utilityUrl: "http://13.201.135.26:1500"
  },
  ivsEnclave: {
...
const createAskAndGetProof = async (
  ...
  try {
    ...
    let inputBytes = abiCoder.encode(["string[3]"], [[input.p_in_1, input.p_in_2, input.p_in_3]]);
...
const createAskAndGetProofPrivate = async (
  createAskAndGetProofParams: createAskAndGetProofParams,
) => {
  try {
    if (
    ...
    let inputBytes = abiCoder.encode(["string[1]"], [[input.p_in_1]]);
...
//Generate Proof for the private market
export const proveTransactionPrivate = async (req: any, res: any) => {
  try {
    let public_input = req.body?.public;
    let secret_input = req.body?.secret;
    let proof = await createAskAndGetProofPrivate({
      pub: public_input,
      sec: secret_input,
    });
    res.status(200).send(proof);
